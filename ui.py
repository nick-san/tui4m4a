...
# init_colors, draw_panes, draw_statusbar は変更なし
def init_colors():
    curses.start_color()
    ...
    curses.init_pair(5, curses.COLOR_BLACK, curses.COLOR_YELLOW) # 確認ダイアログ用
...
def draw_panes(stdscr, app_state):
    ...
    for idx, key in enumerate(tag_keys):
        ...
        if is_selected or is_editing: tag_win.addstr(idx + 1, 1, ' ' * (tag_pane_width - 2), line_style)
...
def draw_statusbar(stdscr, app_state):
    ...
    if "Saved" in status_message: stdscr.addstr(status_bar_y, 1, status_message, curses.color_pair(3))
    else: stdscr.addstr(status_bar_y, 1, status_message)
...
def draw_confirmation_dialog(stdscr, question):
    ...
    dialog_win.bkgd(' ', curses.color_pair(5)) # 背景色を設定
    dialog_win.box()
